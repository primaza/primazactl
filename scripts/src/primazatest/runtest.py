...
def test_main_install(venv_dir, config, cluster):
    ...
    if not "Install and configure main completed" in out:
    ...
    for i in range(1, 60):
        ...
        pods, err = run_cmd(["kubectl", "get", "pods", "-n",
                            "primaza-system", "--context", cluster ], 1 < i < 60)
...
def test_worker_install(venv_dir, config, worker_cluster, main_cluster, private_key):
    ...
    if not "Install and configure worker completed" in out:
        print(f"[FAIL] Unexpected response: {out}")
        return False
...
def main():
    ...
    parser.add_argument("-m", f"--mainclustername",
                        dest="main_cluster_name", type=str,
                        required=True,
                        help=f"name of cluster, as it appears in kubeconfig,"
                             f" on which main is installed. "
                             f"Defaults to worker install cluster.")
    parser.add_argument("-p", "--privatekey",
                        dest="private_key", type=argparse.FileType('r'),
                        required=True,
                        help=f"primaza main private key file. "
                        f"Required for worker install")
